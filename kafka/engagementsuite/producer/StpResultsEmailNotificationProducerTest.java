...
public class StpResultsEmailNotificationProducerTest {
    ...
    private NotificationPropertiesDetails stpResultPropertiesDetails;
    private StpResultsEmailNotificationProducer stpResultsEmailNotificationProducer;

    @BeforeEach
    void setup() {
        stpResultPropertiesDetails = MockHelper.getStpResultsPropertiesDetail();
        NotificationProperties notificationProperties = new NotificationProperties();
        notificationProperties.setStpResultsEmailNotificationProperties(stpResultPropertiesDetails);
        stpResultsEmailNotificationProducer = new StpResultsEmailNotificationProducer(notificationProperties, engagementSuiteGateway);
        ReflectionTestUtils.setField(stpResultsEmailNotificationProducer, "environment", PRD_ENV);
    }
    ...
    void notifyPositive() {
        ...
        stpResultsEmailNotificationProducer.notify(nonStpEmailNotificationInput);
        ...
        assertThat(notificationRequest.getBody().getChannel()).isEqualTo(stpResultPropertiesDetails.getChannel());
        assertThat(notificationRequest.getBody().getSenderAddress()).isEqualTo(stpResultPropertiesDetails.getSenderAddress());
        assertThat(notificationRequest.getBody().getRecipientAddress()).isEqualTo(stpResultPropertiesDetails.getRecipientAddress());
        assertThat(notificationRequest.getBody().getTemplateReference()).isEqualTo(stpResultPropertiesDetails.getTemplateReference());
        assertThat(notificationRequest.getBody().getCommunication()).isEqualTo(SupportedCommunication.STP_RESULTS_EMAIL_NOTIFICATION.name());
    ...
    void notifyError() {
        ...
        try (MockedStatic<InetAddress> inetAddressMockedStatic = mockStatic(InetAddress.class)) {
            ...
            bankGuaranteeRequest.getBgRequest().setStpResultDataSet(stpResultDataset);
            EmailNotificationInput nonStpEmailNotificationInput = MockHelper.getNonStpNotificationInput(bankGuaranteeRequest, Collections.emptyList());
            BgosException exception = assertThrows(BgosException.class, () -> stpResultsEmailNotificationProducer.notify(nonStpEmailNotificationInput));
    ...
    @Test
    void notifyStpResultCCAddressCheck() {
