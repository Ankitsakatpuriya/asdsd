...
public class FulfilmentFailureEmailNotificationProducerTest {
    ...
    void setup() {
        ...
        notificationProperties.setFulfilmentFailureEmailNotificationProperties(nonStpProperties);
        fulfilmentFailureEmailNotificationProducer = new FulfilmentFailureEmailNotificationProducer(notificationProperties, engagementSuiteGateway);
        ReflectionTestUtils.setField(fulfilmentFailureEmailNotificationProducer, "environment", PRD_ENV);
    }
    ...
    void notifyPositive() {
        ...
        fulfilmentFailureEmailNotificationProducer.notify(nonStpEmailNotificationInput);
        ...
        String expectedTemplateName = nonStpProperties.getTemplateReference() + "-" + bankGuaranteeRequestData.getTranslationLanguage().getLanguage();
        assertThat(notificationRequest.getBody().getTemplateReference()).isEqualTo(expectedTemplateName);
        assertThat(notificationRequest.getBody().getCommunication()).isEqualTo(SupportedCommunication.FULFILMENT_FAILURE_EMAIL_NOTIFICATION.name());
    ...
    void notifyError() {
        ...
        try (MockedStatic<InetAddress> inetAddressMockedStatic = mockStatic(InetAddress.class)) {
            ...
            bankGuaranteeRequest.getBgRequest().setStpResultDataSet(stpResultDataset);
            EmailNotificationInput nonStpEmailNotificationInput = MockHelper.getNonStpNotificationInput(bankGuaranteeRequest, List.of(document));
            BgosException exception = assertThrows(BgosException.class, () -> fulfilmentFailureEmailNotificationProducer.notify(nonStpEmailNotificationInput));
    ...
    @Test
    void notifyFulfilmentCCAddressCheck() {
