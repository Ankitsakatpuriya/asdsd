...
class FulfilmentFailureEmailNotificationFeedbackHandlerTest {

    @InjectMocks
    private FulfilmentFailureEmailNotificationFeedbackHandler underTest;
    ...
    @BeforeEach
    void setup() {
        Logger fulfilmentFailureEmailNotificationFeedbackHandler = (Logger) LoggerFactory.getLogger(FulfilmentFailureEmailNotificationFeedbackHandler.class);
        fulfilmentFailureEmailNotificationFeedbackHandler.addAppender(feedbackHandlerLogAppender);
    }

    @Test
    void testHandleSuccess() {
        underTest.handleSuccess(FeedbackEventSamples.getFeedbackEvent(SupportedCommunication.FULFILMENT_FAILURE_EMAIL_NOTIFICATION.name(), SUCCESSFULLY_DELIVERED));
        ...
        verify(feedbackHandlerLogAppender, times(1)).doAppend(argThat((arg) -> arg.getLevel() == Level.INFO && arg.getFormattedMessage()
                .contains("FulfilmentFailureEmailNotificationFeedbackHandler [handleSuccess] Engagement Suite: Email related to Bank guarantee non stp intake failure request has been successfully sent to Customer with id")));
    }

    @Test
    void testHandleFailure() {
        underTest.handleFailure(FeedbackEventSamples.getFeedbackEvent(SupportedCommunication.FULFILMENT_FAILURE_EMAIL_NOTIFICATION.name(),
                NOT_DELIVERED));
        ...
        verify(feedbackHandlerLogAppender, times(2)).doAppend(argThat((arg) -> arg.getLevel() == Level.ERROR
                ...
                .contains("FulfilmentFailureEmailNotificationFeedbackHandler [handleSuccess] Engagement Suite: Email related to Bank guarantee fulfilment failure has not been sent to Customer with id")));
    }
