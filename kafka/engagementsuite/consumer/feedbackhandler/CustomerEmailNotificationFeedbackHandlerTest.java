...
class CustomerEmailNotificationFeedbackHandlerTest {

    @InjectMocks
    private CustomerEmailNotificationFeedbackHandler underTest;
    ...
    @BeforeEach
    void setup() {
        Logger customerEmailNotificationFeedbackHandler = (Logger) LoggerFactory.getLogger(CustomerEmailNotificationFeedbackHandler.class);
        customerEmailNotificationFeedbackHandler.addAppender(feedbackHandlerLogAppender);
    }

    @Test
    void testHandleSuccess() {
        underTest.handleSuccess(FeedbackEventSamples.getFeedbackEvent(SupportedCommunication.CUSTOMER_EMAIL_NOTIFICATION.name(), SUCCESSFULLY_DELIVERED));
        ...
        verify(feedbackHandlerLogAppender, times(1)).doAppend(argThat((arg) -> arg.getLevel() == Level.INFO && arg.getFormattedMessage()
                .contains("CustomerEmailNotificationFeedbackHandler [handleSuccess] Engagement Suite: Email related to Bank guarantee stp request has been successfully sent to Customer with id")));
    }
    ...
    void testHandleFailure() {
        ...
        verify(feedbackHandlerLogAppender, times(2)).doAppend(argThat((arg) -> arg.getLevel() == Level.ERROR
                ...
                .contains("CustomerEmailNotificationFeedbackHandler [handleSuccess] Engagement Suite: Email related to Bank guarantee stp request has not been sent to Customer with id applicationId : failure reason code NOT DELIVERED : message NOT DELIVERED")));
    }
