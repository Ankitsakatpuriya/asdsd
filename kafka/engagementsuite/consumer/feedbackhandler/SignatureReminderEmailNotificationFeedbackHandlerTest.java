...
class SignatureReminderEmailNotificationFeedbackHandlerTest {

    @InjectMocks
    private SignatureReminderEmailNotificationFeedbackHandler underTest;
    ...
    @BeforeEach
    void setup() {
        Logger signCompletionEmailNotificationFeedbackHandler = (Logger) LoggerFactory.getLogger(SignatureReminderEmailNotificationFeedbackHandler.class);
        signCompletionEmailNotificationFeedbackHandler.addAppender(feedbackHandlerLogAppender);
    }

    @Test
    void testHandleSuccess() {
        underTest.handleSuccess(FeedbackEventSamples.getFeedbackEvent(SupportedCommunication.SIGNATURE_REMINDER_EMAIL_NOTIFICATION.name(), SUCCESSFULLY_DELIVERED));
        ...
        verify(feedbackHandlerLogAppender, times(1)).doAppend(argThat((arg) -> arg.getLevel() == Level.INFO && arg.getFormattedMessage()
                .contains("SignatureReminderEmailNotificationFeedbackHandler [handleSuccess] Engagement Suite: Email related to signature reminder email  has been successfully sent to")));
    }

    @Test
    void testHandleFailure() {
        underTest.handleFailure(FeedbackEventSamples.getFeedbackEvent(SupportedCommunication.SIGNATURE_REMINDER_EMAIL_NOTIFICATION.name(),
                NOT_DELIVERED));
        ...
        verify(feedbackHandlerLogAppender, times(2)).doAppend(argThat((arg) -> arg.getLevel() == Level.ERROR && arg.getFormattedMessage()
                .contains("SignatureReminderEmailNotificationFeedbackHandler [handleFailure] Engagement Suite: Email related to signature reminder email has not been sent to")));
    }
